...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
			//timeHtml = '<span class="fc-time">' + htmlEscape(this.getEventTimeText(event)) + '222</span>';
		}
        ...
        var additionalDescr = '';
        if(event.type == 'accItem'){
            var price = parseFloat(event.duration*event.price);
            price = Math.round(price * 100) / 100;

            additionalDescr += '<div class="accItem-calendar-more-content text-left">'+event.category+', '+event.duration+' night, '+event.currency+''+price+'</div>';
            // additionalDescr += '<div class="accItem-calendar-img"><img src="/uploads/'+event.img+'" style="width:50px"></div>';
            // additionalDescr += '<div class="accItem-calendar-duration">'+event.duration+' night</div>';
            // additionalDescr += '<div class="accItem-calendar-price">'+event.currency+''+price+'</div>';
        }
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
			    ...
			    '<div class="accItemCaledarDescr">' + additionalDescr + '</div>' +
				'</div>' +
				(isResizable ?
